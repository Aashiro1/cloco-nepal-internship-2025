    ...
    <SidebarProvider>
      ...
      <SidebarInset>
        ...
        <div className="flex flex-row gap-6 flex-wrap p-6">
          {books.length > 0 ? (
            books.map((book) => (
              <Card key={book.id} className="w-72 cursor-pointer hover:shadow-lg transition" onClick={() => router.push(`/books/${book.id}`)}>
                <CardHeader>
                ...
                <CardFooter className="flex justify-between">
                  {/* <Button onClick={(e) => { e.stopPropagation(); addToCart(book.id); }}>Add to Cart</Button> */}
                </CardFooter>
              </Card>
            ))
          ) : (
